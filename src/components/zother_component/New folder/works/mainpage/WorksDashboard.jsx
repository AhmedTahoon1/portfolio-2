...
const WorksDashboard = () => {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      // Call Backend
      const res = await fetch("http://localhost:5000/works").then((res) =>
        res.json()
      );
      setData(res);
    };
  ...
  const handleShow = async (item) => {
    const fetchApi = async () => {
      // Call Backend
      await fetch(`http://localhost:5000/works/${item.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
...
export default memo(WorksDashboard);
